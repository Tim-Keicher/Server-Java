...
//contain all functions to simulate keyboard
public class keyboardControl {
	...
	//change string into char array and take out every utility char 
	public void writeMessage(String message) {
		createRobot();
		char[] charMessage = message.toCharArray();
		for(int i=0; i<charMessage.length; i++) {
			System.out.println("char i: "+charMessage[i]);
			typeCharacter(charMessage[i]);
		}
	}
	
	//select the respective key press code and run doType function
	public void typeCharacter(char character) {
        switch (character) {
        case 'a': System.out.println(VK_A);doType(VK_A); break;
	...
	//write press key down
    public void doType(int... keyCodes) {
        int length = keyCodes.length;
        for (int i = 0; i < length; i++) {
            rob.keyPress(keyCodes[i]);
        }
        rob.delay(10);
        for (int i = length - 1; i >= 0; i--) {
            rob.keyRelease(keyCodes[i]);
        }
    }
